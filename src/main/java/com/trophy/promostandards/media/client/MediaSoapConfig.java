...
/**
 * Builds the CXF JAX-WS client proxy for the Media Content Service. Only created when
 * {@code promostandards.media.mode=soap}; the proxy is configured from the generated SEI
...
public class MediaSoapConfig {

	@Bean
	MediaContentService mediaSoapPort(PromoStandardsProperties properties) {
		PromoStandardsProperties.Service cfg = properties.getMedia();
		if (cfg.getEndpointUrl() == null || cfg.getEndpointUrl().isBlank()) {
			throw new IllegalStateException(
					"promostandards.media.endpoint-url must be set when promostandards.media.mode=soap");
		}

		JaxWsProxyFactoryBean factory = new JaxWsProxyFactoryBean();
		factory.setServiceClass(MediaContentService.class);
		factory.setAddress(cfg.getEndpointUrl());
		if (cfg.isLogMessages()) {
			factory.getFeatures().add(new LoggingFeature());
		}

		MediaContentService port = factory.create(MediaContentService.class);
